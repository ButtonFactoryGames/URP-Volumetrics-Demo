...
//static int MAX_MARCHING_STEPS = 100;
//static float EPSILON = 0.01f;
//static float END_DISTANCE = 5000;
//static float MINIMUM_MOVE = 0.01f;

//void GetDistance_float(float3 cameraPos, float3 cameraDir, out float alpha, out float thickness, out float3 normals)
//{
//    alpha = 0;
//    thickness = 0;
//    normals = float3(0,0,0);
//    bool inside = false;
//    float3 entryExit = float3(0,0,0);
//    if(_Count != 0)
//    {
//        float start = 0;
//	    float depth = start;
...
//               {
//                   inside = false;
//                   thickness += distance(p, entryExit);
//               }
//            }
//            dist = abs(dist);
...
void GetDistance_float(float3 cameraPos, float3 cameraDir, float pixelDepth, out float voronoi, out float rearVoronoi, out float thickness, out float3 normals)
{
    voronoi = 0;
    rearVoronoi = 0;
    thickness = 0;
    ...
    if(_Count != 0)
    {
        ...
        for (int b = 0; b < MAX_MARCHING_STEPS; b++) 
        {
            ...
            {
                rearVoronoi = (1 - voronoi3D(p,100, 5));
                hitBack = true;
                exit = p;
                break;
            }
